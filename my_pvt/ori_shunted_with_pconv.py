...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if layer == 1 or layer == 2:
            self.dim_conv3 = (4 * dim) // 4
            self.dim_untouched = (4 * dim) - self.dim_conv3

            self.conv1_part = nn.Conv2d(dim, int(3 * dim), 3, 1, 1, bias=False)
            self.conv1_bn = nn.BatchNorm2d(int(4 * dim))
            self.conv1_relu = nn.ReLU(inplace=True)

            self.conv_pconv = nn.Conv2d(self.dim_conv3, self.dim_conv3, 3, 1, 1, bias=False)

            self.conv2_part = nn.Conv2d(int(dim * 4), dim, 3, 1, 1)
    ...
    def forward(self, x, H, W):
        ...
        if self.layer == 1 or self.layer == 2:
            ...
            # part1
            m = self.conv1_part(xx)
            m = torch.cat((m,xx),1)
            m = self.conv1_relu(self.conv1_bn(m))
            # pconv
            x1, x2 = torch.split(m, [self.dim_conv3, self.dim_untouched], dim=1)  # 先split的分开。
            x1 = self.conv_pconv(x1)  # 然后再做pconv
            m = torch.cat((x1, x2), 1)
            # part3
            m = self.conv2_relu(self.conv2_bn(self.conv2_part(m)))
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1:
            ...
            v1 = v1 + self.local_conv1(
                v1.transpose(1, 2).reshape(B, -1, C // 2).transpose(1, 2).view(B, C // 2, H // self.sr_ratio,
                                                                               W // self.sr_ratio)). \
...
@register_model
def ori_shunted_with_pconv(pretrained=False, **kwargs):
