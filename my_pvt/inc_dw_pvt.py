...
#         self.conv_33 = nn.Conv2d(in_channels=dim,out_channels=dim,kernel_size=3,padding=1)
#         self.conv_55 = nn.Conv2d(in_channels=dim,out_channels=dim,kernel_size=5,padding=2)
#         self.conv_77 = nn.Conv2d(in_channels=dim,out_channels=dim,kernel_size=7,padding=3)
#     def forward(self,x):
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if layer == 1 or layer == 2 or layer ==3:
            ...
            #     nn.Conv2d(in_channels=dim, out_channels=int(dim/2), kernel_size=3, padding=1),
            #     nn.BatchNorm2d(int(dim/2)),
            #     nn.ReLU(inplace=True)
            # )
            #
            # self.conv_55 = nn.Sequential(
            #     nn.Conv2d(in_channels=dim, out_channels=int(dim/4), kernel_size=5, padding=2),
            #     nn.BatchNorm2d(int(dim/4)),
            #     nn.ReLU(inplace=True)
            # )
            #
            # self.conv_77 = nn.Sequential(
            #     nn.Conv2d(in_channels=dim, out_channels=int(dim/4), kernel_size=7, padding=3),
            #     nn.BatchNorm2d(int(dim/4)),
            #     nn.ReLU(inplace=True)
            # )
            self.parallelBlock = nn.Sequential(
                nn.BatchNorm2d(dim),
                nn.Conv2d(in_channels=dim,out_channels=int(4*dim),kernel_size=1),
                nn.BatchNorm2d(int(4*dim)),
                nn.ReLU(inplace=True),
                nn.Conv2d(int(4*dim), int(4*dim), 3, 1, 1, bias=True, groups=dim), # DW conv
                nn.BatchNorm2d(int(4 * dim)),
                nn.ReLU(inplace=True),
                nn.Conv2d(in_channels=int(4*dim), out_channels=dim, kernel_size=1),
            )
            # self.conv_33_norm =
            # self.conv_33_relu =
            # self.conv_55 =
            # self.conv_55_norm =
            # self.conv_55_relu = nn.ReLU(inplace=True)
            # self.conv_77 = nn.Conv2d(in_channels=dim, out_channels=dim, kernel_size=7, padding=3)
            # self.conv_77_norm = nn.BatchNorm2d(dim)
            # self.conv_77_relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x, H, W):
        ...
        if self.layer == 1 or self.layer == 2 or self.layer == 3:
            # print('shape: ',x.shape,'--',H,W) # 128,3136,64
            # print('dim:',self.dim)#except 64
            xx = x.reshape(B,H,W,-1).permute(0, 3, 1, 2).contiguous()
            # y = self.conv_33(xx)
            # # print('y',y.shape)
            # z = self.conv_55(xx)
            # # print('z',z.shape)
            ...
            # # print('k',k.shape)
            #
            # m = torch.cat((y,z,k),dim=1)
            # print('m',m.shape)
            # y = torch.cat(k,dim=-1)
            m = self.parallelBlock(xx)
            m = m.permute(0, 2,3,1).contiguous().reshape(B,H*W,-1)
            x = x + self.drop_path(self.attn(self.norm1(x), H, W)) + m
...
class InceptPVTModel(nn.Module):
...
@register_model
def inc_dw_pvt(pretrained=False, **kwargs):
    model = InceptPVTModel(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[1,2,4,1], sr_ratios=[8, 4, 2, 1],
        **kwargs)
