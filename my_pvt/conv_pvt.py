...
class PVTWithConv(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_channels=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.stage_1_conv = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=64, kernel_size=7, stride=1, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=5, stride=1, padding=3),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),
            nn.ReLU(inplace=True),
        ...
        self.stage_2_conv = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=7, stride=1, padding=1, bias=True, groups=64),
            ...
            nn.MaxPool2d(kernel_size=2),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=128,out_channels=128,kernel_size=5,stride=1,padding=3),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True)
        )
        ...
        self.stage_3_conv = nn.Sequential(
            nn.Conv2d(in_channels=128,out_channels=128,kernel_size=7,stride=1,padding=1,bias=True,groups=128),
            # nn.Conv2d(in_channels=128, out_channels=320, kernel_size=7, stride=1, padding=1),
            nn.Conv2d(in_channels=128, out_channels=320, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(320),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),
            nn.ReLU(inplace=True),

            # nn.Conv2d(320, 320, 3, 1, 1, bias=True, groups=320),
            nn.Conv2d(in_channels=320, out_channels=320, kernel_size=5, stride=1, padding=3),
            nn.BatchNorm2d(320),
            nn.ReLU(inplace=True)
        )
        ...
        #     nn.Conv2d(in_channels=320, out_channels=512, kernel_size=7, stride=1, padding=1),
        #     nn.BatchNorm2d(512),
        #     nn.ReLU(inplace=True),
        #     nn.MaxPool2d(kernel_size=2),
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(in_channels=512, out_channels=512, kernel_size=5, stride=1, padding=3),
        #     nn.BatchNorm2d(512),
        #     nn.ReLU(inplace=True)
        # )
    ...
    def forward(self, x):
        ...
        # stage 4
        # x_conv_4 = self.stage_4_conv(x)
        x, (H, W) = self.patch_embed_4(x)  # // 2  # B,320,14,14 -> B,512,7,7 -> B, 49,512
...
@register_model
def conv_pvt_max(pretrained=False, **kwargs):
    model = PVTWithConv(
